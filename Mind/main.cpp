...
|| name.find("BP_SI_ElectricboxB_C") != std::string::npos || name.find("BP_SI_ElectricboxC_C") != std::string::npos
|| name.find("BP_SI_AirdropBox_TreasureBox_Hoarder_C") != std::string::npos || name.find("BP_SI_AirdropBox_Signal_C") != std::string::npos)
{
...
void Render()
{
	...
	if (CFG.b_MenuShow)
	{
		...
		TabButton("Outros", &CFG.tab_index, 2, true);
		TabButton("Config", &CFG.tab_index, 3, true);
		TabButton("Credito", &CFG.tab_index, 4, true);

		ImGui::Separator();

		if (CFG.tab_index == 0)
		{
			ImGui::Checkbox("Marque Para Ativar o ESP", &CFG.b_Visual);
			if (CFG.b_Visual)
			{
				ImGui::Checkbox("Box ESP", &CFG.b_EspBox);
				ImGui::Checkbox("Linhas", &CFG.b_EspLine);
				ImGui::Checkbox("Distancia ESP", &CFG.b_EspDistance);
				ImGui::Checkbox("Vida ESP", &CFG.b_EspHealth);
				ImGui::Checkbox("Arma ESP", &CFG.b_EspShield);
				ImGui::Checkbox("Time ESP", &CFG.b_EspTeam);
				ImGui::Checkbox("Item ESP", &CFG.b_EspItem);
				ImGui::SliderFloat("Distancia Maxima", &CFG.max_distance, 1.0f, 500.0f);
				if (ImGui::ColorEdit3("Cor visivel", CFG.fl_VisibleColor, ImGuiColorEditFlags_NoDragDrop))
				{
					CFG.VisibleColor = ImColor(CFG.fl_VisibleColor[0], CFG.fl_VisibleColor[1], CFG.fl_VisibleColor[2]);
				}
				if (CFG.b_EspBox)
				{
					ImGui::Combo("Tipo de Box", &CFG.BoxType, CFG.BoxTypes, 2);
				}
				if (CFG.b_EspLine)
				{
					ImGui::Combo("Tipo de Linhas", &CFG.LineType, CFG.LineTypes, 3);
				}
			}
		}
		else if (CFG.tab_index == 1)
		{
			ImGui::Checkbox("Marque Para Ativar o Aimbot", &CFG.b_Aimbot);
			ImGui::Checkbox("Ajustar Campo de Visao", &CFG.b_AimbotFOV);
			if (CFG.b_AimbotFOV)
			{
				ImGui::SliderFloat("Puxa Dentro do Circulo", &CFG.AimbotFOV, 1.0f, 150.f);
			...
			ImGui::Combo("Teclado do Aimbot 1", &CFG.AimKey[0], keyItems, IM_ARRAYSIZE(keyItems));
			ImGui::Combo("Tecla do Aimbot 2", &CFG.AimKey[1], keyItems, IM_ARRAYSIZE(keyItems));

			ImGui::SliderFloat("Suavizar Aimbot", &CFG.Smoothing, 1.0f, 20.0f);
			ImGui::Combo("Local do Tiro", &CFG.aimLocation, CFG.aimLocations, IM_ARRAYSIZE(CFG.aimLocations));
		}
		else if (CFG.tab_index == 2)
		{
			...
			ImGui::Combo("Uso de CPU", &CFG.cpuUsage, CFG.cpuUsages, IM_ARRAYSIZE(CFG.cpuUsages));
		}
		else if (CFG.tab_index == 3)
		{
			ImGui::Checkbox("Permitir Tecla de Atalho Fx", &CFG.b_fxHotkey);
			ImGui::Checkbox("Permitir tecla de atalho NUMPAD", &CFG.b_numpadHotkey);
