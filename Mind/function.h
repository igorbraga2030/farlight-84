...
Vector3 ProjectWorldToScreen(Vector3 WorldLocation)
{
	...
	auto CameraCache = read<FMinimalViewInfo>(camera_manager + GameOffset.offset_camera_cache + 0x10);
 
	D3DMATRIX tempMatrix = Matrix(CameraCache.Rotation, Vector3(0, 0, 0));
	...
	Vector3 vDelta = WorldLocation - CameraCache.Location;
	...
	float d = ScreenCenterY / tanf(deg2rad(CameraCache.FOV * 0.5f));
	float FovAngle = (float)(2 * rad2deg(atanf(ScreenCenterX / d)));
