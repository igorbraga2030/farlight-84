...
inline namespace Configuration
{
	...
	public:
		...
		const char* cpuUsages[3] = { "Alto (melhor visual e aimbot)","Medio (ok visual e aimbot)","Baixo (pior visual e aimbot)" };
		const char* BoxTypes[2] = { "Full Box","Cornered Box" };
		const char* LineTypes[3] = { "Bottom To Enemy","Top To Enemy","Crosshair To Enemy" };
		const char* aimLocations[16] = { "Cranio", "Pescoco", "Corpo", "Pelvis", "Thigh right", "Thigh left", "Calf right", "Calf left", "Upperarm right", "Upperarm left", "Lowerarm right", "Lowerarm left", "Hand right", "Hand left", "Foot right", "Foot left" };
